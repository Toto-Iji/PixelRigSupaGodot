...
# Fetch all components with player's collection status
...
# Mark component as collected after level completion
...
# Mark component as installed after minigame completion
...
func collect_component_debug(component_id: String, callback: Callable):
	var path = "/rest/v1/rpc/collect_component"  # Remove _debug
	var body = {
		"p_component_id": component_id
	}
	_do_request(path, HTTPClient.METHOD_POST, body, callback)

func install_component_debug(component_id: String, callback: Callable):
	var path = "/rest/v1/rpc/install_component"  # Remove _debug
	var body = {
		"p_component_id": component_id
	}
	_do_request(path, HTTPClient.METHOD_POST, body, callback)

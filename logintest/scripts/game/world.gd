...
const PAUSE_MENU_SCENE = preload("res://scenes/game/pause_menu.tscn")
const MAIN_MENU_PATH = "res://scenes/main_menu/main_menu_screen.tscn"
const MOBILE_CONTROLS_SCENE = preload("res://scenes/mobile/mobile_controls.tscn")
var mobile_controls = null
var is_paused_active = false
var current_pause_menu = null
# -----------------------------------

var is_death_popup_active = false
...
func _ready():
	...
	# 🆕 Preload quiz questions with dynamic level detection
	var scene_path = get_tree().current_scene.scene_file_path
	var level_id = _extract_level_id(scene_path)
	
	if not level_id.is_empty():
		print("📚 Detected level ID: ", level_id)
		QuizCache.preload_questions(level_id)
	else:
		print("⚠️ Could not detect level ID from scene path, using default")
		QuizCache.preload_questions("level_1")
...
func _setup_mobile_controls():
	...
	if DeviceDetector.is_mobile_device():
		...
		print("🖥️ PC mode - no mobile controls needed")

func respawn_player():
	# This function will now reset our state variable upon execution,
	# though reloading the scene would do this anyway. It's good practice.
	is_death_popup_active = false 
	
	# 1. Unpause the game right before starting the transition.
	get_tree().paused = false

	# 2. Add the transition effect.
	var transition = preload("res://scenes/transition_scene.tscn").instantiate()
	get_tree().root.add_child(transition)

	# 3. Restart the current level smoothly.
	var current_scene_path = get_tree().current_scene.scene_file_path
	transition.start_transition(current_scene_path)
...
func _on_main_menu_requested():
	...
	transition.start_transition("res://scenes/main_menu/main_menu_screen.tscn")
...
func show_death_popup():
	if is_death_popup_active:
		return # Stop the function here

	is_death_popup_active = true
...
# -----------------------------------------------------------------------------
# 🆕 NEW: Dynamic Level ID Extraction
# -----------------------------------------------------------------------------
func _extract_level_id(path: String) -> String:
	var filename = path.get_file().get_basename()
	
	# 🆕 UPDATED: Return the actual database ID format
	# If your scene is "world.tscn" (the first level), return "part_1"
	if filename == "world":
		return "part_1"
	
	# If you have other levels like "part_2_assembly.tscn"
	if filename.begins_with("part_"):
		return filename  # Returns "part_2_assembly" or similar

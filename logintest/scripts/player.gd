...
# --- 💡 NEW: State variable to track if the player is already dead.
...
# --- Constants ---
const BULLET_SCENE = preload("res://scenes/bullet.tscn") # Preload the bullet scene for fast instantiation.
...
func take_damage(amount: int) -> void:
	# --- 💡 MODIFIED: If player is already dead, do nothing.
	if is_dead:
		return
	...
	# --- 💡 MODIFIED: Check if health is zero AND the player isn't already marked as dead.
	if health <= 0 and not is_dead:
		# --- 💡 CRITICAL: Set the flag immediately to prevent this from running twice.
		is_dead = true
		die()
...
func respawn_at(position: Vector2) -> void:
	...
	# --- 💡 ADDED: Reset the death flag so the player can die again in the future.
	is_dead = false 

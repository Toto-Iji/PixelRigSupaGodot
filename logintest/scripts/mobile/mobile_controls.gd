...
func _update_joystick(pos: Vector2):
	var joystick_offset = pos - joystick_center
	if joystick_offset.length() > max_distance:
		joystick_offset = joystick_offset.normalized() * max_distance
	joystick_knob.global_position = joystick_center + joystick_offset - joystick_knob.size / 2
	current_direction = joystick_offset / max_distance
